...
func main() {
	...
	password := ""
	hashdata := strings.ToUpper(fmt.Sprintf("%v", request.Order.OrderID) + fmt.Sprintf("%9v", request.Terminal.ID) + fmt.Sprintf("%v", request.Card.Number) + fmt.Sprintf("%v", request.Transaction.Amount) + password)
